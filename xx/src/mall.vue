      ...
      <div class="mall-new">
        ...
        <div class="new-content">
          <div class="new-lists ">
            ...
            <router-link tag="div" :to="{name:'goodDetail',params:{goodId:newItem.id}}"  class="item two-cell" v-for="newItem in business.news">
...
//          loop:true,
      ...
      init: function() {
        ...
        api.mallHome().then(function(data){
          if (data.status == 200) {
            ...
            vue.support.totalPage=result.totalPages
          }
        },function(err){
          //console.log("err",err);
        })
      ...
      loadMore:function(){
      		var vue = this;
      		if(vue.support.hasMore){
      			vue.support.currentPage++;
      			api.mallHomeNew(vue.support.currentPage).then(function(data){
            if (data.status == 200) {
              var result = data && data.data && data.data.value;
              var resultCode = data && data.data && data.data.result;
              if (resultCode == 0&&result.content.length>0) {
                vue.business.news = vue.business.news.concat(result.content);
              }
              var totalPage = result && result.totalPages;
              if (totalPage <= vue.support.currentPage){
...
<style lang="less" scoped>
  @import url('./assets/css/mall.less');
</style>
